...
ARTICLE_SELECTORS = {
    "bbc.com": [
        "main article div[data-component='text-block'] p",
        "main article p",
    ],
    "theguardian.com": [
        "main article div[data-gu-name='body'] p",
        "main article p",
    ],
    "chinadaily.com.cn": [
        "#Content p",
...
def parse_rss_entries(feed_xml: str, source: str, topic: str) -> list[FeedEntry]:
    ...
    for item in root.findall(".//item"): 
        ...
        if title and link:
            entries.append(FeedEntry(title=title, url=link, source=source, topic=topic))
...
def iter_feed_candidates(config: ScraperConfig) -> Iterable[FeedEntry]:
    for feed in FEEDS:
        ...
        for entry in entries:
            if topic_matches(entry.title, entry.topic):
                yield entry

# 爬取文章
def scrape_article(entry: FeedEntry, timeout_ms: int, retries: int) -> ArticleCandidate | None:
    ...
    with sync_playwright() as playwright:
        ...
        proxy_server = CURRENT_CONFIG.proxy_server
        if proxy_server:
...
def main() -> int:
    ...
    for entry in iter_feed_candidates(config=CURRENT_CONFIG):
        ...
        candidate = scrape_article(entry, timeout_ms=args.timeout_ms, retries=args.retries)
